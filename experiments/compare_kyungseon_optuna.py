#!/usr/bin/env python3
"""Compare same-seed Kyungseon default and validation-selected checkpoints."""
...
def main() -> None:
    ...
    parser.add_argument("--tuned", default="runs/kyungseon_optuna_best")
    ...
    for name, path in (("default", baseline), ("Optuna", tuned)):
    ...
    pid = {
        name: read_csv(path / "pid_response_fixed_bins.csv")
        for name, path in (("default", baseline), ("Optuna", tuned))
    }
    beta = {
        name: read_csv(path / "beta_closure_vs_gen_p.csv")
        for name, path in (("default", baseline), ("Optuna", tuned))
    }
    figure, axes = plt.subplots(2, 3, figsize=(14.5, 8.0), sharex="col")
    for column, (species, label) in enumerate(SPECIES):
        axis = axes[0, column]
        for model, style in (("default", "--s"), ("Optuna", "-^")):
            rows = [
                row
            ...
            x = [
                (float(row["p_low_gev"]) + float(row["p_high_gev"])) / 2
                for row in rows
            ]
            if model == "default":
                axis.plot(
                    x,
                    [float(row["coatjava_fraction"]) for row in rows],
                    "o-",
                    color="black",
                    label="COATJAVA",
                )
            axis.plot(
                x,
                [float(row["fm_mean_probability"]) for row in rows],
                style,
                label=model,
            )
        ...
        for model, style in (("default", "--s"), ("Optuna", "-^")):
            rows = [
                row for row in beta[model] if int(row["generated_pid"]) == species
            ]
            x = [
                (float(row["p_low_gev"]) + float(row["p_high_gev"])) / 2
                for row in rows
            ...
            if model == "default":
                axis.plot(
                    x,
                    [float(row["observed_mean"]) for row in rows],
                    "o-",
                    color="black",
                    label="COATJAVA",
                )
            axis.plot(
                x,
                [float(row["sampled_mean"]) for row in rows],
                style,
                label=model,
            )
        axis.grid(alpha=0.25)
    ...
    figure.suptitle("Kyungseon response: default versus Optuna")
    figure.tight_layout()
    figure.savefig(output / "kyungseon_optuna_test_closure.png", dpi=180)
    plt.close(figure)
